    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                bias_correction2_sqrt = math.sqrt(1 - beta2 ** t)


                denom = vt.sqrt().div_(bias_correction2_sqrt).add_(group['eps']) # \sqrt{\hat{\mathbf{v}}_t} + \epsilon
                #p.data.addcdiv_(mt_hat, denom, value=-lr)
                p.addcdiv_(mt, denom, value=-(lr / bias_correction1))

                if p in self.q3r.grad:
                    p.add_(self.q3r.grad[p], alpha=-lr) #q3r includes regularisation lambda
